...
var (
	...
	stagingGovActivationTime    = uint64(time.Date(2022, time.January, 1, 0, 0, 0, 0, time.UTC).Unix())
...
func GetGovernanceSettingIsActivatedAndCalled(chainID *big.Int, blockTime uint64, to common.Address) bool {
	...
	case chainID.Cmp(params.StagingChainID) == 0 && blockTime >= stagingGovActivationTime:
		switch blockTime {
		default:
			return to == common.HexToAddress("0x1000000000000000000000000000000000000007")
		}
