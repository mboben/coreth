...
func TestBlockGasCost(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:       "before_ap4",
			parentTime: 10,
			upgrades:   params.TestFlareApricotPhase3Config.NetworkUpgrades,
		...
		},
		{
			name:       "normal_ap4",
			parentTime: 10,
			upgrades:   params.TestFlareApricotPhase4Config.NetworkUpgrades,
			parentCost: big.NewInt(ap4.MaxBlockGasCost),
			timestamp:  10 + ap4.TargetBlockRate + 1,
		...
		{
			name:       "normal_ap5",
			upgrades:   params.TestFlareApricotPhase5Config.NetworkUpgrades,
			parentTime: 10,
			parentCost: big.NewInt(ap4.MaxBlockGasCost),
			timestamp:  10 + ap4.TargetBlockRate + 1,
